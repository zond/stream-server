...
- **Where**: **nowhere in particular, and nothing decides it.** A torrent's data is one file per piece under the store's single root (`<cacheRoot>/.pieces/<infoHash>`), for the streaming cache and offline downloads alike, and a pin changes only what is *kept*. A torrent that was streamed first and is then pinned does not move: it is pinned where it is, the engine every open reader holds stays the engine, and `path` in the answer is the same name the backend gave the file before. `settings.downloadsDir` (`POST /settings`, `null` by default) no longer places anything: what it still does is give the cache cleaner a directory to walk *besides* the cache roots, so that whole-file downloads earlier versions wrote there are counted and reclaimed rather than being orphaned and immortal. Set to an absolute path it is checked first, then created if missing (a refused setting leaves no directory behind) and stored resolved, since the cleaner compares roots by plain path prefix and a symlinked spelling would match neither; the update fails if it cannot be created or written, or if it is at or above a cache root — the roots the cleaner walks are the roots it evicts from, and one above the cache root would hand it a tree the engines do not own. A persisted `downloadsDir` that is unusable at startup is cleared with a warning — in the settings file too, so `GET /settings`, the file and the next boot agree. **What is gone with the placement**: a pin used to write its torrent to `<downloadsDir>/<infoHash>` and to *relocate* one that was already managed — dropped from the session, files moved, re-added there, re-checked (`phase: checking`), the hash parked as an in-flight add for the length of a copy that could take minutes, the move detached from the request so a hangup could not strand it, both ends of it held out of the cleaner's reach meanwhile, and the per-hash pin lock handed to the move so an unpin could not delete the tree being written into. None of that exists any more, because none of it moved a byte: the store took one root, so the relocation moved a name.
- **Free space**: a pin is refused (`PinDownloadError::InsufficientSpace`) when the volume its bytes land on has less than the pinned file's missing bytes plus a **500 MiB margin** (`PIN_FREE_SPACE_MARGIN`). That volume is the piece store's root, for every torrent, and there is no longer any other candidate: nothing places a torrent anywhere, so the one volume a pin can write to is the one the store is on. (It used to probe `<downloadsDir>/<infoHash>`, where no payload byte was ever written — passing a pin onto a full store and refusing one that had all the room it needed.) Only the missing bytes are asked for, because that is all a pin writes. Re-pinning a complete file needs nothing, and a torrent still `checking` data that may already be there (right after a restart, or a stream's add the pin joined) is not measured at all — its `downloaded` reads 0 until the check ends and its want-set is already librqbit's. A whole-file download an earlier version left on disk is *not* data in place: this session neither converts nor reads it, so a fresh pin over it is measured for the whole file. A refused pin **drops the torrent it added and leaves every byte where it is**: the refusal comes before anything is downloaded, so its own add wrote nothing, while what the store holds for that hash was fetched by an earlier stream or an earlier session and is cache for the cleaner. A torrent the pin merely joined — a stream request's in-flight add — is not dropped at all, since a reader is about to open on it; nor is one a reader took hold of *while the pin was checking*, which is a real window (the engine is published before the check, and the check reads the file list and the stats before it ever probes free space) and one that the join count cannot see, so the live activity registers are consulted as well.
- **Cache cleaner**: the 30-day and `cacheSize` rules apply to every root it walks — the cache root and `downloadsDir` alike. **The cap it enforces is the smaller of `cacheSize` and what the filesystem can give**: `cacheSize` unset is `u64::MAX`, which on a 4 GB television meant nothing was ever evicted and librqbit wrote until the filesystem refused — an ENOSPC that arrives as a fatal torrent error, mid-film. So each pass reads the volume's free space (`fs4::available_space` → `statvfs`: the raw `statfs` syscall on Linux, bionic's `statvfs` on Android, both `f_frsize * f_bavail`, i.e. `df`'s "Available") and caps the cache at `occupancy + available − 512 MiB`, keeping that 512 MiB floor free. The floor is the same number `routes::stream::ensure_download_disk_ready` demands before it will stream to disk at all — below it that check runs one cleaner pass of its own and, if the disk is still short, answers the stream `507 Insufficient Storage` — so it is exactly the line the cleaner keeps the cache out of. (It used to say it "degraded the request to memory-only"; there is no memory-only engine, and the fallback re-selected the same disk-backed one.) A volume that cannot be probed is not read as "full": the configured `cacheSize` then stands alone. **A cap is a statement about one volume, so the walked roots get one budget per volume** (`cache_cleaner::budgets_by_volume`, over `enginefs::volume_id`) — normally one, since the cache roots and `downloadsDir` normally share a disk. Where they do not, summing every root's occupancy against the tightest free-space reading of all of them would have the cleaner evicting a healthy system disk's cache to answer a shortage on an external download drive it can reclaim nothing from, because what fills a download drive is pinned. `cacheSize` is a per-volume allowance by the same argument. `POST /cache/clean` and `GET /cache.json` still answer with one report: sizes add, and the reported limit adds too — except that one uncapped volume leaves the whole run's limit `null`, since its occupancy is in the total with nothing bounding it. Whether some volume is *still over* is a separate field, `overLimit` (how far over the pass ended, summed across the volumes that are), because summed `total` against summed `limit` cannot answer it — a system disk's slack would pay off a full download drive and report a device that is stuck as fine. It is what the shortfall line is built from, and what a client should read before telling a user cleaning helped. `downloadsDir` is walked and counted like any other root — the cleaner reads the configured value from the settings, and walking it is now the only thing that value does; it used to be pruned out of the walk, which left an old plain-file download there orphaned *and* immortal once downloads became piece files — and a `downloadsDir` inside the cache root is one walk of the outer root, not two, **when the two are on the same volume**. The roots are grouped by volume *before* they are collapsed, because collapsing first is what would hide the case the budgets exist for: `downloadsDir` is allowed to sit below a cache root and on a desktop it is an external drive mounted there, so a `downloadsDir` on another volume stays a root of its own with its own cap, and each walk stops where the other's root begins (`WalkDir` crosses mount points, and a walk that crossed would count and evict the drive's files against the cache root's free space all over again). What survives there survives because it is *protected*, never because of where it lives: a live engine's piece directory (`<cacheRoot>/.pieces/<infoHash>`, which since the piece store became the session's default storage is where all of a torrent's data is), and the same directory for a **dormant** pin, which has no engine to report anything. A whole-file download an earlier version wrote is *not* protected: there is no migration, the torrent that owns it re-downloads as pieces, and those bytes are ordinary cache the cleaner ages out like any other. The session's own records in the cache root (`session.json`, `<infoHash>.torrent`, the `.bitv` bitfields, `pinned-downloads.json` and their temp files) are never cache either. What `/proxy` cached is (`.proxy` under the cache root, see [Proxied remote streams](#proxied-remote-streams)): it is walked, counted, aged and evicted like anything else, and deliberately nowhere near `protected_paths`. **Sizes are what a file occupies, not its apparent length**: librqbit's filesystem storage pre-allocates every file it wants at full size, so a part-streamed film is a multi-gigabyte `length` over a handful of allocated blocks — counting `length` once reported 17 GB of cache on a phone holding 3.85 GB. The piece store this server runs on now pre-allocates nothing, but the roots are still full of what earlier versions wrote and nothing migrates, so the rule stands. On Unix the cleaner uses `st_blocks`; Windows has no cheap equivalent through `std`, so the apparent length still stands in there (an over-estimate, which errs towards cleaning rather than towards a full disk). A run that cannot get under the limit says so, with how many bytes protection is holding — nothing a live torrent is writing or a pin keeps is ever evicted. **A full disk is a signal to clean, not to stop**: librqbit treats a write that hits `ENOSPC` as fatal and leaves the torrent in an error state, which is how a film died ninety minutes in against a swarm of 459 seeds. The cleaner therefore checks every 15 s for torrents stopped for want of space (`EngineFS::out_of_space_torrents` — one lock read per engine, no I/O; a stopped torrent writes nothing, so there is no filesystem event left to arm the debounce with), runs a pass, and restarts them (`EngineFS::restart_after_error` → librqbit's `Session::unpause`, whose error-state path re-checks what is on disk before going live, so an eviction that took files is discovered rather than assumed). It restarts them only if the pass actually freed something — restarting onto a still-full disk would just reproduce the error. **The engine stops a torrent before the filesystem has to**: the floor used to be consulted when a stream started and never again, and librqbit writes the whole file it wants, so a film larger than the free space ran a 4 GB television to zero in 40 s and died at `ENOSPC` with the rest of the device failing around it. The engine's free-space arm (the reconciler's, every 2 s, one `statvfs` of the volume every torrent's pieces land on) now pauses a torrent that is writing (`Session::pause`: peers dropped, writes stopped, files and piece map kept — pinned torrents included, since a pin is a reason to keep the bytes, not a licence to run the disk out) the moment the volume falls under the floor, and rings the cleaner, which runs its pass at once rather than on the next poll — so the torrent's readers see a buffering pause of a few seconds while the pass decides, not a dead stream. A recovery pass evicts to 256 MiB under the cap (`RECOVERY_HEADROOM`) so the restarted torrent has room to run before the watch stops it again; the watch resumes a stopped torrent on its own once the volume is 64 MiB over the floor. Its `stats.json` says `phase: "error"` with a fixed "stopped for want of disk space" message while it is stopped, and a `GET` for it is a `507`. **What a stopped torrent's bytes are worth depends on what else can go.** A torrent in the error state for any reason *other* than space is dead — nothing will restart it, the error would only recur — and its files are cache the pass evicts first, before a film somebody might watch again. A torrent stopped *for* space is listed separately (`EvictionClasses::stopped_for_space`) and is neither protected nor unlinked: while the pass can evict anything else, that is the room the stopped torrent resumes into, and the film being watched is worth more than one nobody is; only when the pass could free nothing else does the stopped torrent go — whole, through its engine (`EngineFS::evict_stopped_torrent`: torrent, files and piece map together, since a paused torrent holds its files open and a piece map that says it has them), its readers failed, and the hash refused with a `507` (`MagnetAddError::EvictedForSpace`) for 30 s so a player's reconnect or a stats poll cannot re-add it and refill the disk the pass just emptied. That is what a user retrying the same title needs: before this the previous attempt's corpse was protected as a live engine's files and the retry failed for the room it held. The residual is stated plainly: a film larger than the free space still cannot play to the end without a rolling piece window. Torrent data is one file per piece now (the piece store is the session's default storage), so reclaiming a single piece is a `remove_file` and the window is buildable; the retention policy that would drive it is still unwired, for a reason argued in `enginefs/src/piece_store/mod.rs`. Until then such a film fails a few MB under the floor with the rest of the device intact, and the next title plays. It runs on its own schedule (debounced after filesystem activity, and hourly as a fallback) — `GET /cache.json` and `POST /cache/clean` are how a client reads its state and triggers a pass on demand, without restarting the server. See [Cache usage and cleaning](#cache-usage-and-cleaning).
...
- Offline downloads add files next to the session state in `<cache dir>/rqbit-downloads`: `pinned-downloads.json` (the pin set) and one `<infoHash>.bitv` per torrent (fastresume bitfields — the first start after upgrading still hash-checks each torrent once, after which restarts skip it). The cache cleaner leaves all of them alone. `settings.downloadsDir` defaults to `null` and places nothing: it only adds a directory to the cache cleaner's walk (see [Offline downloads](#offline-downloads)).
