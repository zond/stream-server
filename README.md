...
- **Library API**: an embedder calls `ServerHandle` methods directly -- the same code the HTTP routes run, no HTTP client needed. See [Library API](#library-api)
- **Stats**: `/stats.json`, `/{infoHash}/stats.json`, `/{infoHash}/{fileIdx}/stats.json` for server status and torrent progress
- **Settings**: runtime-configurable via `/settings`, with the stremio-core-compatible shape
...
- **Media routes are OPEN.** They hand bytes to a player. Players (mpv, or a Chromecast receiver through the [LAN media listener](#lan-media-listener)) fetch the URLs stremio-core builds for them (`types/resource/stream.rs`) and cannot attach headers, so these routes take no token.
...
| Method | Same as |
|---|---|
| `auth_token() -> Option<&str>` | the token control routes require; a started server always has one |
