...
| `large` | 4 min | 4 MiB |
| `maximum` | no time cap — the whole file, as far as the cache budget reaches | 4 MiB |
...
The cache bounds itself (see *What bounds the cache* above); these two calls let a client show its usage and ask for its slack back without a restart:
