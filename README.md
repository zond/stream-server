...
**The `Authorization`, `Cookie` and `Proxy-Authorization` in `h=` travel no further than they would on a hop this route walks itself, and on a cleartext line not that far**: a rewritten line *is* a hop, made by the player and written by us, so a line naming `http://` where the playlist came over `https` is written without them, and a playlist fetched over an already-downgraded chain arms none of its lines with them, `https` lines included. That is what makes the drop above stick — without it an `https` origin naming an `http` segment had the caller's credentials written into the segment's URL and delivered to the cleartext host by the player's own next fetch, one line after the redirect guard had withheld them.

A playlist that arrived over cleartext because the *caller* named an `http://` target keeps them — **on the lines naming its own origin, and on no others**. The credential has been spent in the clear on exactly one origin: a line pointing back at that origin publishes nothing that is not published already, and an authenticated plain-`http` stream would lose every segment without it, while a line naming anywhere else is a fresh disclosure the *origin* chose and the player makes without asking. Scoping that exception by the playlist's **scheme** instead, which is how it first shipped, armed every `http` line in a cleartext playlist: measured, a caller naming `http://A/master.m3u8` with `h=Authorization:Bearer …` got back a playlist naming `http://B/seg-0.ts`, and B logged the credential when the line was fetched the way a player fetches it — and it re-armed at every nesting level (A naming B's playlist, B naming `http://C/seg-c.ts`, C getting it), because a rewritten line is not a hop of the request that wrote it but a fresh one, so the ten-hop bound covers none of it. "Its own origin" is what `d=` is written with: scheme, host **and port** — a subdomain is a different host, and a different port is a different listener that may be a different party, while the default port spelled out is neither. This is deliberately stricter than the rule for a hop, which does carry the credential across a cleartext `302` to another host: there the origin the caller named said the resource had *moved*, whereas a playlist line merely names a part of it.
