...
**Why this server speaks HTTP at all, and to whom.** This crate is a library the app links; the app calls it through `ServerHandle` ([Library API](#library-api)) and never opens an HTTP connection to it. HTTP exists for exactly two callers, and `build_router()` (`server/src/lib.rs`) is split along that line:
...
| GET | `/drive/stream/{key}` | OPEN | players -- the bytes of a file in a paired Google Drive, by range. The key is random and the URL carries **no credential**: the grant is an argument to `ServerHandle::open_drive_file` and is spent inside the server for an hourly access token. A dead pairing is `401` `{"refused":"pairAgain"}`, never a generic `502`. See [Google Drive files](#google-drive-files) |
...
| GET | `/{infoHash}/{fileIdx}/stats.json` | TOKEN | stremio-core `Statistics`; accepts `tr=`/`f=` like the stream route |
...
| GET | `/downloads/{key}/stream` | OPEN | players -- a finished (or filling) non-torrent download, served by range off the pinned proxy-cache entry; the `playUrl` of its `DownloadInfo`. See [Offline downloads](#offline-downloads) |

Every TOKEN row is a path stremio-core's fork calls; every OPEN row is a URL a player fetches. That is the complete list -- there is no "app" row, because the app calls the [Library API](#library-api). Unknown paths get `404`, a wrong method on a known path `405` (or `401` first, on a control route).
...
**Every container is read as byte ranges and nothing is stored.** `/{zip|tar|rar|7zip|iso}/create` with `urls` reads the archive's index through the same ranged reads and the same cache `/proxy` uses -- the end record, the central directory, one local header per stored member for a ZIP; one 512-byte header per member for a TAR; each volume's headers for a RAR; the signature header and the packed index at the end for a 7z; the descriptors and the directory tree for an image -- and remembers *that*: an index in memory, no file. A member's bytes are then a range of the archive, so a range request for the film is one ranged request at the origin, a seek is one more, and **nothing is ever written under the cache root**. The `torrent:` form (`/{fmt}/stream/torrent:<infoHash>%2F<path>/{member}`) is the same thing over a torrent's own file: a seek in the member is a seek in the piece store. **The index is kept for as long as the bytes it indexes are** -- while nothing else has been opened -- and not for a fixed idle time: a player or a cast receiver that has paused still has its `{key}`, and what takes the session is the viewer starting something else, the same event that makes the cached bytes disposable. An origin that answers a ranged request with the whole file is **refused** with `501` and a sentence saying so -- serving it would mean downloading the archive to reach the member, which is the thing this design exists to stop.
...
**A RAR set is its volumes, in order.** A film in a RAR is a film across `film.part1.rar`, `film.part2.rar`, ... (or `film.rar`, `film.r00`, `film.r01`, ...) far more often than in one file, and a member of a set is a *part* of each volume. `/rar/create` takes the `urls` list as the volume list in the order it was given -- which is what stremio-core builds from an addon's `rarUrls` -- and the `torrent:` form finds the volumes among the sibling files of the one named, by those two naming rules and nothing else. The member is then one range of each volume, and a range request that crosses a volume boundary is answered from both. A set with a gap in its run is `422`, **naming the volume it wanted**, rather than a film with a silent hole in it; a file named under neither rule is one volume. Stored members only: a compressed, encrypted or solid RAR is refused like any other (above), and an encrypted stored member is refused too, whose bytes the layout could map -- there is no password UX, and a wrong guess is ciphertext to the player.

**A 7z is refused unless it is stored.** `/7zip/create` reads the archive's index the same way as the rest -- the 32-byte signature header at the front and the packed index at the back, which is where 7z keeps it -- and a file whose block is a single COPY coder with no encryption is byte ranges of the archive, like a stored ZIP member. Everything else is `415`: LZMA2 (what `7z a` writes by default, and what a 7z of a film off the internet nearly always is) as `compressed` naming the method, an AES coder as `encrypted`, and a block that is not COPY and holds several files as `solid` -- reaching the second file there means decoding the first, so no decoder could enter at it either. A multi-part set (`.7z.001`, `.7z.002`, ...) is `422` saying it is one file cut up: joining the parts is fetching all of them.
...
| `cacheSize` | `10737418240` (10 GiB) | Bytes the cache may hold; `null` is unlimited. The cap actually enforced is the smaller of this and what the volume can give -- see [What bounds the cache](#what-bounds-the-cache) |
...
|---|---|---|
| `normal` (default) | 90 s (`BufferProfile::window_seconds`) | 4 MiB |
| `large` | 4 min | 4 MiB |
| `maximum` | a day (`MAXIMUM_WINDOW_SECONDS`) -- in effect the whole file, as far as the cache budget reaches | 4 MiB |

**The unit is seconds of film, not bytes of disk.** How far a playing stream reads ahead is the film's own bitrate -- its size over the duration a player stated (`ServerHandle::note_duration`) -- times the profile's seconds, and the same number sizes the retention window that keeps those bytes, so what the swarm is asked for and what the disk is kept for are one answer with one source. Sized from the disk instead, as they were, the forward reach was a fraction of `cacheSize`, so a viewer who gave the app a bigger cache silently bought a bigger mobile-data bill.
...
**This is the control surface.** An embedder holds a `ServerHandle` (from `stream_server::start`) and does everything through it; there is no HTTP client in the app and, for most of these, no route at all. A method that needs the server's runtime runs on it and blocks the calling thread until done (a few -- `set_background`, `close_proxy_streams`, `dht_status`, `proxy_streams_live` -- answer synchronously without one); all returned types are `serde`-serializable, so they can be passed as JSON over FFI. Where a method shares its function with one of the core-protocol routes, the "Same as" column says which, so the core and the app can never see two servers:

| Method | Same as |
|---|---|
| `auth_token() -> Option<&str>` | the token the core-protocol routes require; the app attaches it in `Env::fetch`. A started server always has one |
...
| `note_duration(info_hash, file_idx: &str, filters: &[String], duration)` | no route; `async`, awaited on the caller's runtime -- a player states how long the film is, with the file named as its stream URL spells it (`-1` and `f=` included). The stream's read-ahead and its retention window are sized from that duration, in seconds of film; see [Buffer profiles](#buffer-profiles) |
| `engine_stats(info_hash, trackers: &[String]) -> Result<EngineStats>` | no route -- torrent-level stats for the guessed stream file, including creating the engine with `trackers` when it is the first request for the hash and answering `resolvingMetadata` at once. `trackers` are normalised inside the shared function exactly like the routes' `tr=` (`tracker:` prefix stripped, `dht:` dropped, trimmed), so a stream's `sources` array can be passed as is |
...
| `pin_download(info_hash, file_idx: usize, trackers) -> Result<DownloadInfo>` | no route -- pin the file as an offline download (see [Offline downloads](#offline-downloads)); `trackers` are normalised as for `engine_stats` |
| `unpin_download(info_hash, file_idx: usize, delete_files: bool) -> Result<UnpinOutcome>` | no route -- drop the pin, and with `delete_files` the data; `unpinned: false` when nothing was pinned, `deleted_files` what actually left the disk |
| `pin_proxy_download(ProxyDownloadRequest) -> Result<DownloadInfo>` / `unpin_proxy_download(key, delete_files) -> Result<UnpinOutcome>` / `proxy_download_key(&ProxyPinKey) -> Option<String>` | no route -- the same for an addon URL or a Drive file, keyed by the server's cache key (the row's `infoHash`); see [Offline downloads](#offline-downloads) and `docs/generic-downloads.md` |
| `downloads() -> Result<Vec<DownloadInfo>>` | no route -- every pinned download, torrent and proxy rows alike |
...
| `cache_usage() -> Result<CacheUsage>` | no route -- what the cache occupies against its limit right now, without deleting anything. See [Cache usage and cleaning](#cache-usage-and-cleaning) |
| `clean_cache_now() -> Result<EvictionReport>` | no route -- drop both owners' slack immediately and report what is left; a pin and the window of the stream being played are never touched. See [Cache usage and cleaning](#cache-usage-and-cleaning) |
| `stream_numbers(url: &str) -> Result<Option<StreamNumbers>>` | no route -- the cache around the playhead and, for a torrent, the committed set, this session's transfer totals (fetched, unverified, uploaded and their ratio) and the engine's refused reclaims (each absent where there is no such number, never zeroed). `None` is a stream this server does not hold, which is not an error. See [What a panel is told about a stream](#what-a-panel-is-told-about-a-stream) |
| `close_proxy_streams(token: &str) -> usize` | no route -- end every proxied stream the client marked with `token`, retire the token, and answer how many streams that was. See [Ending a proxied stream](#ending-a-proxied-stream) |
| `open_drive_file(file_id, refresh_token, name) -> Result<Result<DriveFileOpened, DriveOpenError>>` | no route -- open a file in a paired Google Drive and get a `url` a player can fetch, absolute on this server. **How an embedder asks, and why the credential never becomes a request**: the grant is an argument to a function call in this process, not a query string, a path segment, a body or a logged line. `DriveOpenError::is_pair_again()` is the one thing a caller must be able to tell without reading English -- its answer is a new pairing, and everything else's is to try again. See [Google Drive files](#google-drive-files) |
| `dht_status() -> DhtStatus` | no route -- see [DHT health](#dht-health-serverhandledht_status) |
...
Where a core-protocol route and a method answer the same question (`/settings`, the per-file stats), they call one function (`routes::system::{file_stats, update_settings}`), so they cannot drift; `server/tests/embed.rs` compares them. Everything else here has no HTTP twin to drift from.
...
- **Pin**: `ServerHandle::pin_download(info_hash, file_idx, trackers)`. `trackers` may be empty; a stream's own `sources`/`announce` array can be passed as is, and, as everywhere else, it only matters when this call is the one that creates the engine. Idempotent. The answer is one `DownloadInfo`: `{infoHash, fileIdx, path, name, length, downloaded, complete, phase, error}` (and, for a non-torrent download, `source` and `playUrl`). A file the torrent does not have is `PinDownloadError::FileNotFound`, a disk without room `InsufficientSpace`, a magnet that never resolved `MagnetAdd` -- `PinDownloadError::client_message` is the sentence to show, and it names no local path.
- **List**: `ServerHandle::downloads()` -- the same `DownloadInfo` shape, one entry per pinned file, ordered by info hash then file index, torrent rows first and non-torrent rows after them. A dormant pin (see *Restarts* below) is listed last with `phase: "error"` and an `error` explaining that its torrent is not managed right now; `path`/`length`/`downloaded` are then unknown (`null`/`0`). `ServerHandle::download_path(info_hash, file_idx)` returns just the `path` without creating anything. It is a name and not a file: torrent data is stored one file per piece, so nothing is ever written at that path and a client plays a completed download through the media routes like any other.
- **Unpin**: `ServerHandle::unpin_download(info_hash, file_idx, delete_files)` answers an `UnpinOutcome {unpinned, deleted_files}`; `unpinned: false` means nothing was pinned, and `deleted_files` reports what actually left the disk rather than echoing the flag (a failed delete is logged, not raised). Without `delete_files` only the pin goes: the bytes stay where they are and the torrent becomes an ordinary, evictable one again (with nothing playing, the next reconciler tick stops the torrent and its bytes are given back like any unpinned file's). With `delete_files` the data goes too -- the whole torrent (files, session record and its now-empty folder) when this was its last pin and nothing is reading the torrent, only the one file while other files of it stay pinned or another file of it is streaming, since the torrent must keep running for them. Deleting does not require the file to have been pinned, but it does require the file to exist: with `delete_files`, a `file_idx` the torrent does not have is `FileNotFound` exactly as it is for a pin, never a request to delete the whole torrent. A per-file delete is two deletions, because a torrent's bytes are piece files: the pieces the backend agrees to give up (`drop_pieces` forgets them and frees nothing -- the indices come back so that the caller can delete them), and, if an earlier version of this server left one there, the whole-file copy at the path the backend reports. That file is truncated before it is unlinked, because librqbit holds an open handle on every file of a running torrent and an unlink alone would free no space -- and it stops counting as the torrent's active file first, so the want-set re-planned around it cannot select it again and start refilling the unlinked inode (deleting the file you are watching frees the disk; the stream itself simply ends). `deletedFiles` answers for both and means bytes actually left the disk: a delete that found nothing to take says `false` rather than reading "already absent" as "freed". Note what "the whole torrent" costs: deleting the **last** pin drops files that were only ever streamed along with the pinned one. It does not drop a torrent something is reading: the activity registers are asked first (a season pack whose pinned episode 1 is deleted while episode 2 plays keeps running, and loses only episode 1), and what is left is ordinary evictable cache once the reads end. An unpin that keeps the files leaves the bytes in place, of course. A pin whose torrent the backend does not have (see *Restarts*) still has its directory in the piece store: `delete_files` removes that once no file of that torrent is pinned any more, and the entry leaves `downloads()` with the pin. The launch sweep takes every directory no pin claims, so those bytes were never immortal -- but while the pin stands the sweep keeps that directory, so an explicit delete is the only thing that takes it at once. **"The backend does not have it" is checked, not inferred from the engine registry**: the registry can lose an engine while the session keeps the torrent running (the idle sweep mid-step), and a magnet add parks a hash outside both for as long as metadata takes. So a `delete_files` unpin that finds no engine deletes the piece directory by hand only for a hash the session has never heard of; a torrent it still holds is deleted through the session instead, which drops the torrent before its storage releases the pieces, and a hash being added right now is left alone and answers `deletedFiles: false`. Without a pin to remove there is nothing to delete either -- the bytes belong to no download the server knows of, and the next launch's sweep takes them. A hash nothing ever downloaded has nothing in the store, and the answer then says `deletedFiles: false` rather than reading "already absent" as "freed".
- **Where**: **nowhere in particular, and nothing decides it.** A torrent's data is one file per piece under the store's single root (`<cacheRoot>/rqbit-downloads/.pieces/<infoHash>`), for the streaming cache and offline downloads alike, and a pin changes only what is *kept*. A torrent that was streamed first and is then pinned does not move: it is pinned where it is, the engine every open reader holds stays the engine, and `path` in the answer is the same name the backend gave the file before. There is exactly one torrent-data root and it is `settings.cacheRoot` (`update_settings`): set to an absolute path it is checked first, then created if missing (a refused setting leaves no directory behind) and stored resolved, since the session is opened on it and everything else is spelled the way the engine reports it. The update fails if it is not a string, is relative, or cannot be created or written. **A running librqbit session cannot be moved onto a new root**, so a change takes effect at the next start; at that start the value is prepared *before* anything opens on it, and one that cannot be used any more falls back to the configured cache directory -- persisted too, so `settings()`, the settings file and the next boot agree. `settings.downloadsDir` is gone: it was a second location for pinned downloads, there is no second-card use case, and a pin does not move a torrent, so the key named a place that never held anything. A client that still sends it gets what it gets for any unknown settings key -- nothing.
...
A torrent the free-space arm stopped says `phase: "error"` in its `stats.json`, with a fixed "stopped for want of disk space" message, for as long as it is stopped, and a `GET` for it drops both owners' slack and, if the volume is still short, is a `507`.

**A film larger than the free space plays anyway**, which is what the rolling piece window is for. Torrent data is one file per piece (the piece store is the session's default storage), so reclaiming a single piece is a `remove_file`, and `enginefs/src/retention.rs` drives it from what the reads are doing, on the reconciler's two-second tick: every consumer the read-pattern detector finds on the file -- a viewer, a second audio track, a player's index crawler -- is kept and fetched a lookahead ahead of where it has reached, the film's own bitrate times the seconds the buffer profile asks for, shared out between them when the disk cannot cover them all; up to half the budget is a set committed for sharing; and what puts the file over its allowance is given back, coldest first -- the backend told to forget it first, then the bytes, under one claim. Bytes behind a consumer are not given back on a schedule: they are simply the first to go when the room is needed, which is what makes a scrub back cheap without anyone having to decide how far back a viewer might go.
...
The cache bounds itself (see [What bounds the cache](#what-bounds-the-cache) above); these two calls let a client show its usage and ask for its slack back without a restart:

- **Usage**: `ServerHandle::cache_usage()` reports a `CacheUsage` without deleting anything: `{totalBytes, limitBytes, protectedBytes, protectedFiles}`. `totalBytes` is occupancy (`st_blocks * 512` on Unix, not apparent length), and it comes from the owners that hold the bytes rather than from a walk: a sum over the bits each live piece store keeps, plus the proxy cache's running count, plus one `read_dir` of the store root for what no live store speaks for (a torrent held in Error, a previous process's leftovers), plus the staged copies of the pieces a live store is still writing, one `stat` each. `limitBytes` is the limit actually enforced in the same accounting -- the smaller of `settings.cacheSize` and what the volume holding the cache can give while keeping the free-space floor free, so on a small device it is a number even when `cacheSize` is `null` (`null` only when neither caps anything: `cacheSize` unlimited *and* the volume's free space unreadable). `protectedBytes`/`protectedFiles` are what a pin keeps or the stream being played is inside -- the part nothing can take. When `protectedBytes` equals `totalBytes` and the cache is still over `limitBytes`, cleaning cannot help: there is nothing disposable left until playback moves on or something is unpinned, and that is the line to give a user, not "clean failed". It costs no walk, one `read_dir` and a `stat` per piece in flight, so calling it when a "Storage" screen opens or is refreshed is cheap; it is not cached server-side, so avoid polling it on a sub-second timer.
- **Clean now**: `ServerHandle::clean_cache_now()` gives back everything nobody is playing and nobody is reading, now -- the same passes the reconciler's tick and a viewer opening something else run, so it can never be less careful -- and answers an `EvictionReport`: `{total, protected, protectedFiles, freed, deleted, limit, overLimit}`, occupancy throughout. **It is not a choice of victims**: a pin is kept until it is unpinned and the window of the one entity being played is kept until something else is played, so on a device with one film playing and one pinned the honest answer is that nothing was freed. `freed`/`deleted` are what this call really took; `total` is what the cache occupies once it finished, which can still be over `limit`. The cap is restated before the report is built (`cache_budget::publish_now`), so the owners are sized against the volume as the clean left it. `limit` is the number `cache_usage()` answers, which is the same rule applied to a different figure: that total also counts what no owner's count prices -- a torrent held in Error, a directory a previous run left, the pieces in flight -- and the published cap does not, so with any of those on the disk the reported `limit` is larger than the cap the owners are sized against, and `overLimit` is the excess of the whole root rather than of what the owners hold. A client offering "clean now" should read the response the same way as `cache_usage()`'s shortfall case: if `total` is still over `limit` afterwards, say that the reachable cache is as small as it gets right now, because a pin or the stream being played is holding the rest -- not that the clean failed.

Both are `cache_cleaner::{usage, drop_slack}`, the same passes the reconciler's tick runs; neither has a route.

### Proxied remote streams

`/proxy` fetches a URL the caller names and relays it to the player, adding the headers the addon said it needs. It is how an addon stream that is not a torrent reaches the player at all, and it is open (players cannot send a bearer header). It is deliberately absent from the [LAN media listener](#lan-media-listener), because it will fetch any URL whoever reaches it names; on a server bound to loopback, which is the default, only this device can reach it -- but that is every app on the device and every page a browser on it has open, not just the embedder -- and a host that binds the main listener to `0.0.0.0` makes it reachable from the whole LAN (see [Quick Start](#quick-start)). What keeps a web page from *reading* the LAN through it is that the loopback listener answers no CORS at all (no `Access-Control-Allow-Origin`, on any route, and `r=` cannot add one): a page can still make the request, but it cannot see the answer. Nothing that reads loopback is a browser, so nothing needed the wildcard it used to answer.
...
**Certificates are verified, and a chain nothing trusts is refused.** This route was built with verification off from its first commit -- inherited from the closed-source proxy it was ported from, and nothing in the history names a host that needed it -- and once every remote stream went through here, that flag was how every remote stream was fetched. For a while after that, a host whose certificate would not verify was retried once with verification off; that downgrade is gone (`enginefs::http_client`). An on-path attacker can produce a certificate error as easily as a misconfigured CDN can, so a downgrade on error stopped nothing it could not also trigger. The roots are Mozilla's, compiled in, plus the platform's own certificates, so a device or organisation that has installed its own CA keeps working. Redirects are walked hop by hop (below), so a failed handshake is reported against the hop that failed.
...
**Proxied bytes are cached, in whole 256 KiB chunks.** This route relayed and kept nothing, which was measured rather than assumed: the same range fetched twice went to the origin twice. That was tolerable while the player kept a disk cache of its own; it stopped being tolerable once the client began routing every remote stream through here so that it would not have to, and every backward seek past the player's memory became a fresh origin fetch. So `/proxy` now keeps what it fetches, under `<cacheRoot>/rqbit-downloads/.proxy`, beside the piece store and inside the cache root -- so every byte of it is in the one usage figure and under the one published cap, with its own retention owner reclaiming it and nothing of it pinned. A proxied stream nobody is reading is the first thing that should go. A fill never writes the volume under the free-space floor: the chunk that would is not kept, and the player carries on from the origin (see [What bounds the cache](#what-bounds-the-cache)).

**And it is bounded by the same retention policy a torrent is.** Where a proxied stream has got to is written from bytes that really reached a player, on both the cached and the fetched path, and never from a `Range` header, and the same read-pattern detector the torrent's owner uses says how many consumers a body has and where each has reached. What is kept is the stretch each one is being fetched into -- seconds of film at the response's own bitrate, bounded by the published cap -- and what puts the response over the cap is given back coldest first, as playback moves on. So a film streamed through `/proxy` never has more than the budget of it on disk, however long it is, and a short seek back is answered from the disk instead of from the origin, because what is behind a player is given up only when the room is wanted for something else. One thing differs from the torrent's half of it and it follows from there being no swarm: nothing is committed for sharing, so the whole budget is window. Two things are the same fact seen from the other side, and both are here because there is no backend to refuse a delete the way librqbit refuses one for a piece a reader is waiting on. **The chunks under a live player's window are nobody's to take** -- and where nothing is being reclaimed at all (the budget covers the response, the volume has no cap and no `cacheSize` is set) a live player is inside *all* of them, exactly as a torrent with no policy announces everything it holds and gives up none of it. **And an open read holds the bytes it has already promised**: a response is framed before its first byte goes out, so the chunks between a hit's first and last are owed to that player until they have gone out, and no pass may unlink one -- a window reaches forward from where a player has got to, so a body longer than that has its own tail outside it from its first chunk onwards, and this is what stops the cache truncating a read it told the player it held. Two players inside one response are two playheads and get two windows, so neither reclaims the other's read-ahead. Nothing about a playhead is persisted or guessed: before a byte has gone out this process has no idea where anyone is in a cached response, and a response nobody is playing and nobody is reading holds nothing at all -- its chunks go the moment a stream opens on something else.

**And a player is read ahead of, as a torrent stream is.** The window above is what the owner *keeps*; until 2026-09-26 nothing *fetched* it -- a proxied body was pulled by the player's own `Range` and by nothing else, so a Drive or debrid stream had a torrent stream's retention and none of its buffer. Now the proxy backing fills the window the same pass draws (`proxy_retention::Prefetcher`, the proxy's half of what a torrent's picker does with the want set): each pass hands the backing its want-windows, the chunks the disk does not hold are fetched through a quiet source over the entity's own URL and credentials -- quiet, so the filler claims no live entity and leaves no playhead, and is never mistaken for a viewer -- and they land in the cache ahead of the player. Where the budget covers the response whole the owner installs no window and runs no pass, exactly as an unbounded torrent is left to its picker; then the player's own delivered bytes drive it, and what is fetched is the rest of the file from the head. **Only a player's stream is read ahead of**: the `/proxy` request has to carry the client's player token (`p=`, which the app puts on every URL it hands mpv), and a Drive session is a player's by construction. A request without one -- a probe, an archive's index read, a thumbnail, a test -- fetches exactly what it asked for, as before; a measured rate cannot be the gate, since one 256 KiB read is enough to measure one. It stops on its own: a stride at a time (32 MiB), looking between strides at whether a newer ask has arrived (a seek re-targets it) and at whether the entity is still the one being played (a viewer who went elsewhere stops it), and a minute with nothing asked ends the task. What it costs is origin traffic -- on Drive against the file's download quota, on a debrid link against whatever the addon meters -- bounded by the window when the budget bounds the response and by the file when it does not, which is the torrent's own bound.

**Ranges are the whole of what a request fetches.** A player seeks; it does not download -- the read-ahead above is the one thing that fetches more than was asked, and it fetches on the player's behalf. The cache is asked before a socket is opened, and it answers one of three things: the range is here, and it is served off disk with the origin never learning the read happened; part of it is here, and the `Range` that goes on the wire is narrowed to the missing part, from the chunk boundary the cache ended at, with the cached head written in front of the origin's tail where the two prove to be one entity, below (the player is still answered about the whole range it asked for); or none of it is, and the fetch goes out as asked. A miss streams to the player **as it fills** -- the writer sits under the same registry the body is read through, so a close or a client that vanished ends the fill at the byte it ends the read.
...
So the client names them. It mints a token per player and puts it in the `/proxy` URL that player is given, as `p=` in either URL shape (see [Proxied remote streams](#proxied-remote-streams)).

The token is the proxy's own parameter, like `d=`, `h=` and `r=`: it is never sent to the origin. Any string the client can generate works -- it is a name, not a credential, and the call that uses it is a method on the handle, inside the process. Every playlist this proxy rewrites carries the token into the segment URLs it writes, so an HLS player's segment fetches belong to the same token as its playlist.

Then `ServerHandle::close_proxy_streams(token)` ends every live stream carrying that token and answers how many that was. Zero is an ordinary answer -- the player may already have finished -- and closing twice is harmless. There is no route for it, on either listener: the ability to cut playback is not something to hand the network.
...
That includes a request that is already in flight. `/proxy` checks the token before it opens the origin, which makes a refusal free, but the origin then takes as long as it takes: a close landing during one time-to-first-byte used to answer `0` -- nothing was registered yet -- and the stream it did not see went on to be served in full, measured at 3.9 MB. Registering a stream and retiring a token are decided under one lock now, so a fetch in flight is either counted by the close or refused `410` when it comes back.

**Close after the player has been told to quit, not before.** A demuxer that has been cancelled never reaches its reconnect -- ffmpeg checks its interrupt callback before the retry delay, before every read and inside the socket poll -- so the close arrives to find nothing live and answers `0`, which is the outcome to want. Closing *first* races the cancel, and losing that race provokes exactly the reconnect the refusal then has to catch: one more origin connection on the way out. A client whose teardown is a quit followed by a close never needs the refusal; it is there for the case where the quit does not arrive, or does not work.
...
client secret lives -- never on the device); `routes::drive` is the open
and the one route.

| | |
|---|---|
| **The open** | `ServerHandle::open_drive_file(file_id, refresh_token, name)`, **a call, not a request**. The refresh token is an argument inside the process -- not a query parameter, not a path segment, not a body -- because the request line is the half of a request that gets logged, by the tracing layer here, by whatever an embedder puts in front, and by the diagnostics report a viewer can copy, and a body is one more copy in flight. `/proxy`'s `h=Authorization:…` overrides ride in the path, so they are not reused; they are also the wrong shape, since what a Drive file needs is a *grant* spent for a new header every hour rather than a header value relayed verbatim |
...
| **Read ahead of** | A playing Drive file is read ahead of through the session's own source, quietly, exactly as a `/proxy` stream is -- see "And a player is read ahead of" under [Proxied remote streams](#proxied-remote-streams). A file the budget covers whole is fetched to its end from the head; a larger one is fetched a window ahead. Drive traffic, against the file's quota, bounded the same way |
| **Downloaded, and offline** | A Drive file pinned as a download (`pin_proxy_download` with `driveFileId` + `refreshToken`; see [Offline downloads](#offline-downloads)) is filled through the same `DriveSource` and kept in the proxy cache under `ProxyPinKey::Drive`. Once it is whole, `open_drive_file` answers **from the disk first**: the download's own route `/downloads/{key}/stream`, the length and type the disk holds, and no request to Drive or to the pairing service -- so it plays on a device with no network, and a launch that re-pins it opens nothing either. A file that is not downloaded is opened through Drive as before |
...
The `POST /drive/create` that once took the grant in a JSON body went
with the other control routes the app never used (2026-09-26); see
...
fetch an archive's index from a caller-named URL, and the loopback stream route's first request for
...
That is tolerable on an embedded server's loopback listener -- which is not
"only this app": every app on the device reaches it, and so does every page a
browser on it has open, which is why loopback answers no CORS and a page can
make these requests but never read their answers -- but not on a listener the
whole LAN can reach, so none of them is on the LAN allow-list. (A host that binds the main listener to
...
**2026-09-26: the control routes that mirrored the embed API.** `/heartbeat`, `/stats.json` (with its `?sys=1` probe), `/{infoHash}/stats.json`, `POST`/`DELETE /{infoHash}/{fileIdx}/download`, `GET /downloads.json`, `POST /downloads`, `DELETE /downloads/{key}`, `GET /cache.json`, `POST /cache/clean`, `GET /stream-numbers.json`, `POST /proxy-streams/{token}/close` and `POST /drive/create`. Each was a token-protected HTTP twin of a `ServerHandle` method, kept from the standalone daemon this fork started as. The app calls the methods over FFI and never had an HTTP client; stremio-core never called any of them; the only caller was this repository's own tests, which now call the handle. Keeping them meant a second front door to every capability -- one that took a credential in a body (`/drive/create`), one that could cut another device's playback (`/proxy-streams`), one that deleted files (`DELETE …/download`) -- for nobody. The methods are unchanged; see [Library API](#library-api). What remains of the control router is exactly the streaming-server protocol the core fork speaks, listed under [API](#api), and the rule there says why nothing gets added to it.
...
- `/diagnostics/*` -- local debugging endpoints. The periodic memory sampler that logged alongside them is gone too; the process figures it read are now taken only by the panic hook, which puts them in the crash line.
...
- **2026-09-26. Proxied and Drive streams are read ahead of.** A stream a player is reading through `/proxy` (with its `p=` token) or `/drive/stream` now fetches the retention window ahead of the player -- or the rest of the file, where the budget covers it whole -- instead of only what the player's `Range` asked for. Origin traffic per stream goes up by that much, on Drive against the file's quota; nothing on disk changes, and requests without a player token fetch exactly what they ask for as before.
- **2026-09-26. The app-facing control routes are gone** (`/heartbeat`, `/stats.json`, `/{infoHash}/stats.json`, the download, cache, stream-numbers, proxy-streams and `/drive/create` routes -- the list is under [Removed routes](#removed-routes)). An embedder that called any of them over HTTP calls the matching `ServerHandle` method instead; the core-protocol routes and every media route are unchanged. Nothing on disk changes.
- **2026-09-03.** Offline downloads add one `<infoHash>.bitv` per torrent next to the session state in `<cacheRoot>/rqbit-downloads` (fastresume bitfields -- the first start after upgrading still hash-checks each torrent once, after which restarts skip it). The pin set itself is not written here at all: the embedder keeps it and hands it in at startup. Nothing reclaims either: the launch sweep also clears `rqbit-downloads` of a previous release's data, but it exempts the session's records by name, and a `pinned-downloads.json` an older build left is kept where it lies rather than swept as a previous release's data. `settings.downloadsDir` is gone; the one torrent-data root is `settings.cacheRoot`, which now decides where the session opens (see [Offline downloads](#offline-downloads)). A client that still sends the old key gets what any unknown settings key gets -- nothing -- and the key is dropped from `settings.json` on the next save.
- **2026-09-04.** The DHT bootstrap address cache is a new file, `<cacheRoot>/rqbit-downloads/dht-bootstrap.json`, written next to `dht.json`. It holds only the IP addresses the bootstrap host names last resolved to, is rewritten when what they resolve to changes, and is safe to delete -- it is a fallback for a network whose DNS is broken, not state anything depends on.
- **2026-09-08. Torrent data is stored one file per piece**, under `<cacheRoot>/rqbit-downloads/.pieces/<infoHash>/`, for the streaming cache and offline downloads alike: whole `.mkv` files are not produced any more. **There is no migration, and the old bytes are deleted, not converted.** The torrent that owns a download written by an earlier version comes up with nothing on its first start and downloads again as pieces. The old file sits beside the piece store, not under it, so no owner speaks for it and no usage figure ever includes it -- and **the launch sweep removes it whole** (`piece_store::sweep_legacy_downloads`) on the first start that is handed a pin set, because a byte nobody counts and nothing deletes is how a disk fills once and never empties. The same sweep removes the `.cache` (a pre-fork server's `<hash>.torrent` files) and `.metadata` directories from `rqbit-downloads`. A start handed no pin set sweeps nothing, that included. Delete that directory by hand if you want the space back, or unpin the download with `deleteFiles=1`, which is the one call that still knows the path. Nothing breaks, no path a client already holds stops resolving, and `DownloadInfo::path` keeps its shape -- but it names a file that will not appear, so a client that opened it directly must go through the media routes instead.
