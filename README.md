...
**A hit and a miss are classified the same way.** The classification that decides whether a body is a rewritten playlist is one function, and a full hit asks it before it answers rather than after; a hit whose verdict is "playlist" steps aside and the origin is fetched. That is what keeps `r=` out of the key honestly: while the hit answered first, `r=Content-Type:application/x-mpegURL` — what stremio-core sends for an HLS stream — was rewritten on a miss and relayed raw on a hit, so the second player's segments went straight to the origin without the `h=` they needed.
