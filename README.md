...
- **What "not watching" means**: no open stream and no live playback lease, over the window as well as right now. The bytes a player's own stream pulled in stay in the counters after playback ends, so a signal that only asked about *now* would accuse the background of the viewer's own film every time they stopped it. The cost of getting that right is that after playback ends the light can take up to two windows to come on for traffic that really is unattended.
