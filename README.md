...
**It is deliberately narrow, and this is the whole of what it does not do:** no revalidation of any kind (no `ETag`, no `If-None-Match`, no `Last-Modified`, no freshness lifetime — an entry is served until the cleaner evicts it, and an origin that changes the entity under the same URL without changing its length or its content type is served stale), no `Vary`, no coalescing of two players filling the same chunk, and nothing at all cached for a fetch that starts mid-chunk. It also refuses, each for its own reason:
...
| an entity whose length the origin will not state | there is nothing to file the chunks under and no `Content-Range` a hit could write |
| `Cache-Control: no-store`, `no-cache`, `private`, or `max-age=0` | the first two say so outright; the last two say "revalidate first", and this never revalidates, so for it they say the same thing. **This header was read nowhere in the route before the cache existed** — the rule is introduced, not inherited |
...
`r=` is not part of the key: it never reaches the origin, so it varies the response the player is handed and not one byte of what the origin sends, and what is stored is origin bytes replayed through the same classification and header code. `p=` is not part of it either — each player mints its own, and two players reading one stream at two offsets are exactly the case the cache exists for.
