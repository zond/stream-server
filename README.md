...
- Offline downloads add files next to the session state in `<cache dir>/rqbit-downloads`: `pinned-downloads.json` (the pin set) and one `<infoHash>.bitv` per torrent (fastresume bitfields — the first start after upgrading still hash-checks each torrent once, after which restarts skip it). The cache cleaner leaves all of them alone. `settings.downloadsDir` defaults to `null` and setting it only affects torrents pinned from then on (an already-pinned torrent relocates on its next pin).
