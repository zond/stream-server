...
`/proxy` fetches a URL the caller names and relays it to the player, adding the headers the addon said it needs. It is how an addon stream that is not a torrent reaches the player at all, and it is open (players cannot send a bearer header). It is deliberately absent from the [LAN media listener](#lan-media-listener), because it will fetch any URL whoever reaches it names; on a server bound to loopback, which is the default, only this device can reach it, and a host that binds the main listener to `0.0.0.0` makes it reachable from the whole LAN (see [Quick Start](#quick-start)).
