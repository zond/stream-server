...
A **pinned** file stays wanted no matter which file of the torrent is being played, and its torrent is exempt from idle removal and the seeding-disabled pause for as long as it has a pinned file — which also keeps it out of the cache cleaner's reach: the cleaner protects every live engine's piece directory, and a pinned engine stays live. **Pinning is a retention property, not a location**: a pinned download is not written anywhere else, does not move, and is stored one file per piece under the store's single root exactly like a torrent that is only being streamed. `stats.json` reports `pinnedFiles` and, per file, `pinned` and `complete` (`downloaded == length`).
...
- **Free space**: a pin is refused (`PinDownloadError::InsufficientSpace`) when the volume its bytes land on has less than the pinned file's missing bytes plus a **500 MiB margin** (`PIN_FREE_SPACE_MARGIN`). That volume is the piece store's root, for every torrent, and there is no longer any other candidate: nothing places a torrent anywhere, so the one volume a pin can write to is the one the store is on. (It used to probe the pin's own placement folder, where no payload byte was ever written — passing a pin onto a full store and refusing one that had all the room it needed.) Only the missing bytes are asked for, because that is all a pin writes. Re-pinning a complete file needs nothing, and a torrent still `checking` data that may already be there (right after a restart, or a stream's add the pin joined) is not measured at all — its `downloaded` reads 0 until the check ends and its want-set is already librqbit's. A whole-file download an earlier version left on disk is *not* data in place: this session neither converts nor reads it, so a fresh pin over it is measured for the whole file. A refused pin **drops the torrent it added and leaves every byte where it is**: the refusal comes before anything is downloaded, so its own add wrote nothing, while what the store holds for that hash was fetched by an earlier stream or an earlier session and is cache for the cleaner. A torrent the pin merely joined — a stream request's in-flight add — is not dropped at all, since a reader is about to open on it; nor is one a reader took hold of *while the pin was checking*, which is a real window (the engine is published before the check, and the check reads the file list and the stats before it ever probes free space) and one that the join count cannot see, so the live activity registers are consulted as well.
- **Cache cleaner**: the 30-day and `cacheSize` rules apply to the whole of the one root it covers — `settings.cacheRoot`, taken from the running engine rather than from the setting, since a root set for the next start is not where the bytes are now. **The cap it enforces is the smaller of `cacheSize` and what the filesystem can give**: `cacheSize` unset is `u64::MAX`, which on a 4 GB television meant nothing was ever evicted and librqbit wrote until the filesystem refused — an ENOSPC that arrives as a fatal torrent error, mid-film. So each pass reads the volume's free space (`fs4::available_space` → `statvfs`: the raw `statfs` syscall on Linux, bionic's `statvfs` on Android, both `f_frsize * f_bavail`, i.e. `df`'s "Available") and caps the cache at `occupancy + available − 512 MiB`, keeping that 512 MiB floor free. The floor is the same number `routes::stream::ensure_download_disk_ready` demands before it will stream to disk at all — below it that check runs one cleaner pass of its own and, if the disk is still short, answers the stream `507 Insufficient Storage` — so it is exactly the line the cleaner keeps the cache out of. (It used to say it "degraded the request to memory-only"; there is no memory-only engine, and the fallback re-selected the same disk-backed one.) A volume that cannot be probed is not read as "full": the configured `cacheSize` then stands alone. **A cap is a statement about one volume, and there is one root, so there is one cap.** There used to be a budget per volume, grouped before the roots were collapsed into each other, because `downloadsDir` could put a second root on an external drive: summing both roots' occupancy against the tightest free-space reading of the two would have the cleaner evicting a healthy system disk's cache to answer a shortage on a drive it can reclaim nothing from, because what fills a download drive is pinned. With one root that whole apparatus (`budgets_by_volume`, `outermost`, the walk boundaries, the report fold) has nothing to describe and is gone. `overLimit` stays as a field of its own — how far over the cap the pass ended — because it is what a client should read before telling a user that cleaning helped, and "still stuck" should be one number rather than a comparison every reader has to get right. A plain-file download an earlier version wrote is walked and counted like anything else under the root; it used to be possible to put such downloads *outside* the walk, which left them orphaned *and* immortal once downloads became piece files. **The cleaner walks everything under the root except the piece store, and asks the store what is in there.** Torrent payload is one file per piece under a directory shape (`.pieces/<infoHash>/<bucket>/<piece>`) that `enginefs::piece_store::StoreRoot` alone knows: the cleaner is handed an info hash and a piece index, cannot build the path, and unlinks none of the store's files — it asks the store to let a piece go. So a piece and the half-written copy of a re-download over it are one thing, freed together and reported as one deletion, and a file in the store the store does not recognise is counted (it is on the disk) and left for the startup sweep rather than unlinked by a pass that cannot know what it is. What survives survives because the retention policy will not give it up, never because of where it lives — and the question is asked about **one piece of one torrent**, not about a path: *have we told a peer about this piece?* A torrent nothing is streaming announces everything it holds, so nothing of it goes, and that covers every live engine, every pinned one and every **dormant** pin, which has no engine to report anything. A torrent something *is* streaming announces only the pieces its playback window has settled on, and the rest — the read-ahead, the cache behind the playhead — is the cleaner's to take. A whole-file download an earlier version wrote is *not* protected: there is no migration, the torrent that owns it re-downloads as pieces, and those bytes are ordinary cache the cleaner ages out like any other. The session's own records in the cache root (`session.json`, `<infoHash>.torrent`, the `.bitv` bitfields, `pinned-downloads.json` and their temp files) are never cache either. What `/proxy` cached is (`.proxy` under the cache root, see [Proxied remote streams](#proxied-remote-streams)): it is walked, counted, aged and evicted like anything else, and the one thing that keeps a chunk of it is the same question asked of a torrent's piece — is somebody inside these bytes right now? A proxied stream a player is playing through holds the window round its playhead, and whatever an open body has been framed to deliver and has not delivered yet; one nobody is reading holds nothing, and place protects nothing here. **Sizes are what a file occupies, not its apparent length**: librqbit's filesystem storage pre-allocates every file it wants at full size, so a part-streamed film is a multi-gigabyte `length` over a handful of allocated blocks — counting `length` once reported 17 GB of cache on a phone holding 3.85 GB. The piece store this server runs on now pre-allocates nothing, but the roots are still full of what earlier versions wrote and nothing migrates, so the rule stands. On Unix the cleaner uses `st_blocks`; Windows has no cheap equivalent through `std`, so the apparent length still stands in there (an over-estimate, which errs towards cleaning rather than towards a full disk). A run that cannot get under the limit says so, with how many bytes protection is holding — nothing a live torrent is writing or a pin keeps is ever evicted. **A full disk is a signal to clean, not to stop**: librqbit treats a write that hits `ENOSPC` as fatal and leaves the torrent in an error state, which is how a film died ninety minutes in against a swarm of 459 seeds. The cleaner therefore checks every 15 s for torrents stopped for want of space (`EngineFS::out_of_space_torrents` — one lock read per engine, no I/O; a stopped torrent writes nothing, so there is no filesystem event left to arm the debounce with), runs a pass, and restarts them (`EngineFS::restart_after_error` → librqbit's `Session::unpause`, whose error-state path re-checks what is on disk before going live, so an eviction that took files is discovered rather than assumed). It restarts them only if the pass actually freed something — restarting onto a still-full disk would just reproduce the error. **The engine stops a torrent before the filesystem has to**: the floor used to be consulted when a stream started and never again, and librqbit writes the whole file it wants, so a film larger than the free space ran a 4 GB television to zero in 40 s and died at `ENOSPC` with the rest of the device failing around it. The engine's free-space arm (the reconciler's, every 2 s, one `statvfs` of the volume every torrent's pieces land on) now pauses a torrent that is writing (`Session::pause`: peers dropped, writes stopped, files and piece map kept — pinned torrents included, since a pin is a reason to keep the bytes, not a licence to run the disk out) the moment the volume falls under the floor, and rings the cleaner, which runs its pass at once rather than on the next poll — so the torrent's readers see a buffering pause of a few seconds while the pass decides, not a dead stream. A recovery pass evicts to 256 MiB under the cap (`RECOVERY_HEADROOM`) so the restarted torrent has room to run before the watch stops it again; the watch resumes a stopped torrent on its own once the volume is 64 MiB over the floor. Its `stats.json` says `phase: "error"` with a fixed "stopped for want of disk space" message while it is stopped, and a `GET` for it is a `507`. **What a stopped torrent's bytes are worth depends on what else can go.** A torrent in the error state for any reason *other* than space is dead — nothing will restart it, the error would only recur — and its files are cache the pass evicts first, before a film somebody might watch again. A torrent stopped *for* space is listed separately (`ReclaimVerdicts::stopped_for_space`) and is never taken a piece at a time — it keeps its piece map and announces it again the moment it resumes: while the pass can evict anything else, that is the room the stopped torrent resumes into, and the film being watched is worth more than one nobody is; only when the pass could free nothing else does the stopped torrent go — whole, through its engine (`EngineFS::evict_stopped_torrent`: torrent, files and piece map together, since a paused torrent holds its files open and a piece map that says it has them), its readers failed, and the hash refused with a `507` (`MagnetAddError::EvictedForSpace`) for 30 s so a player's reconnect or a stats poll cannot re-add it and refill the disk the pass just emptied. That is what a user retrying the same title needs: before this the previous attempt's corpse was protected as a live engine's files and the retry failed for the room it held. **A film larger than the free space plays anyway**, which is what the rolling piece window is for. Torrent data is one file per piece (the piece store is the session's default storage), so reclaiming a single piece is a `remove_file`, and `enginefs/src/retention.rs` drives it from the playhead on the reconciler's two-second tick: half the cache budget is a window around where the player is, half is a set committed for sharing, and everything else of the file being streamed is given back — the backend told to forget it first, then the bytes, under one claim. **Only what is committed is ever announced to a peer**, so this server never advertises a piece it is about to throw away. **The budget is not the cleaner's, and does not wait for a pass**: `server/src/cache_budget.rs` states it from one `statvfs` and the last occupancy anything counted (0 before anything has, which is the tightest cap the volume could justify and therefore the safe way to be wrong), on its own minute timer, at startup and whenever a client changes `cacheSize` — so a stream relayed in the seconds before the first walk of a sixteen-thousand-file cache is bounded like any other, and lowering the setting takes effect at once rather than at the next pass. A pass is one more caller of that publisher, differing only in having just counted the cache; every publication is ordered against the others (`CachePasses`), because a reading taken before `cacheSize` was lowered must not land on top of one taken after. The cleaner runs on its own schedule (debounced after filesystem activity, and hourly as a fallback) — `GET /cache.json` and `POST /cache/clean` are how a client reads its state and triggers a pass on demand, without restarting the server. See [Cache usage and cleaning](#cache-usage-and-cleaning).
- **Restarts**: librqbit persists each torrent's place and want-set and, with fastresume, its verified pieces (`<cacheRoot>/rqbit-downloads/<infoHash>.bitv`), so a pinned download resumes where it was without a full re-hash; the pin set itself is persisted in `<cacheRoot>/rqbit-downloads/pinned-downloads.json` and restored at startup. A pin whose torrent the session did not bring back (a record librqbit could not restore — an unparseable `.torrent`, an add that errored) stays in that file, dormant, until the torrent returns — on a later boot, or with the next pin of the same torrent, which applies the dormant pins alongside its own — or until it is unpinned.

### Cache usage and cleaning

The cache cleaner (above) runs on its own schedule and had no way to be asked about or triggered from outside the process — a client wanting to show cache usage, or offer a "clean now" action, had to restart the server just to make its start-up sweep fire, which stops playback. These two calls replace that:

- **Usage**: `GET /cache.json` (`ServerHandle::cache_usage`) reports a `CacheUsage` without evicting anything: `{totalBytes, limitBytes, protectedBytes, protectedFiles}`. `totalBytes` is occupancy exactly as the cleaner counts it (`st_blocks * 512` on Unix, not apparent length — see the cache cleaner bullet above), `limitBytes` is the limit actually enforced in the same accounting — the smaller of `settings.cacheSize` and what the volume holding the cache can give while keeping 512 MiB free, so on a small device it is a number even when `cacheSize` is `null` (`null` only when neither caps anything: `cacheSize` unlimited *and* the volume's free space unreadable), and `protectedBytes`/`protectedFiles` are what a live engine is writing or a pin keeps — the part a clean pass can never take. When `protectedBytes` equals `totalBytes` and the cache is still over `limitBytes`, cleaning cannot help: there is nothing left to evict until playback stops or something is unpinned, and that is the line to give a user, not "clean failed" — a client should show `totalBytes` against `limitBytes` and, once over, name `protectedBytes` as what is currently unreachable rather than retrying the clean. The walk costs one `stat` per file currently in the cache, no reads — the same walk the cleaner itself already does on every debounced or hourly pass — so calling it once when a "Storage" screen opens or is refreshed is cheap; it is not bounded or cached server-side, so avoid polling it on a sub-second timer.
- **Clean now**: `POST /cache/clean` (`ServerHandle::clean_cache_now`) runs one eviction pass immediately — the identical function the scheduled sweep calls, so it can never be less careful — and answers an `EvictionReport`: `{total, protected, protectedFiles, freed, deleted, limit}`, occupancy throughout. `freed`/`deleted` are what this pass actually reclaimed; `total` is what the walked roots occupy once it finished, which can still be over `limit` when everything left is protected. A client offering "clean now" should read the response the same way as `cache.json`'s shortfall case: if `total` is still over `limit` after cleaning, say that the reachable cache is as small as it gets right now, because a live engine or a pinned download is holding the rest — not that the clean failed.
...
**Presence means complete, and that had to be built.** The piece store can afford a weaker claim because librqbit hash-checks every piece against the swarm's own SHA-1; a URL response has no hashes to check against. So a chunk is held in memory until it is whole, written to a temporary name and renamed into place, and a chunk file whose length disagrees with the entity it is filed under is refused at the read and deleted, never served (the lookup goes by the names in a bucket directory, one listing per thousand chunks, not a stat per chunk). A client that goes away mid-chunk leaves nothing on disk at all, and the launch-time sweep exists only to take the temporaries a kill left behind — everything else there is cache, and surviving a restart is the point of it.

**It is deliberately narrow, and this is the whole of what it does not do:** nothing here is revalidated — no `If-None-Match`, no `If-Modified-Since`, no freshness lifetime, and an entry is served until the cleaner evicts it. The validator above is kept and compared, but only ever on a fetch the player had asked for anyway; a read the store answers in full asks the origin nothing, so a resource that changed under the URL is served as it was until this key next misses, and it is the fill that misses which discovers the change and drops what was held of the old entity. There is no `Vary`, no coalescing of two players filling the same chunk, and nothing kept of the chunk a fetch starts inside — the bytes before a body's first chunk boundary are dropped, since there is no completing a chunk whose front the response does not carry, while every whole chunk after it is written as usual. It also refuses, each for its own reason:
...
- Offline downloads add files next to the session state in `<cache dir>/rqbit-downloads`: `pinned-downloads.json` (the pin set) and one `<infoHash>.bitv` per torrent (fastresume bitfields — the first start after upgrading still hash-checks each torrent once, after which restarts skip it). The cache cleaner leaves all of them alone. `settings.downloadsDir` is gone; the one torrent-data root is `settings.cacheRoot`, which now decides where the session opens (see [Offline downloads](#offline-downloads)). A client that still sends the old key gets what any unknown settings key gets — nothing — and the key is dropped from `settings.json` on the next save.
- **Torrent data is stored one file per piece**, under `<cache dir>/rqbit-downloads/.pieces/<infoHash>/`, for the streaming cache and offline downloads alike: whole `.mkv` files are not produced any more. **There is no migration and nothing moves on upgrade.** A download written by an earlier version stays exactly where it is; the torrent that owns it comes up with nothing on its first start and downloads again as pieces, and the old bytes are ordinary cache the cleaner ages out. Nothing breaks, no path a client already holds stops resolving, and `DownloadInfo::path` keeps its shape — but it names a file that will not appear, so a client that opened it directly must go through the media routes instead.
