...
**The `Authorization`, `Cookie` and `Proxy-Authorization` in `h=` travel only as far as they would on a hop this route walks itself**: a rewritten line *is* a hop, made by the player and written by us, so a line naming `http://` where the playlist came over `https` is written without them, and a playlist fetched over an already-downgraded chain arms none of its lines with them, `https` lines included. That is what makes the drop above stick — without it an `https` origin naming an `http` segment had the caller's credentials written into the segment's URL and delivered to the cleartext host by the player's own next fetch, one line after the redirect guard had withheld them. A playlist that arrived over cleartext because the *caller* named an `http://` target keeps them on its cleartext lines: the same exception the loop makes for the first hop, and an authenticated plain-`http` stream would otherwise lose every segment for a secret already spent on that wire.
