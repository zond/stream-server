...
A **pinned** file stays wanted no matter which file of the torrent is being played, and its torrent is exempt from idle removal and the seeding-disabled pause for as long as it has a pinned file — and nothing ever reclaims a byte of it: a pin is kept until it is unpinned, whatever the cache is doing. **Pinning is a retention property, not a location**: a pinned download is not written anywhere else, does not move, and is stored one file per piece under the store's single root exactly like a torrent that is only being streamed. `stats.json` reports `pinnedFiles` and, per file, `pinned` and `complete` (`downloaded == length`).
