...
Asking is cheap — two atomics per torrent that exists and the three live playback fields, nothing built — and it is a peek, not a poll: it touches no torrent's idle clock, so it cannot keep anything out of the idle sweep, and it creates nothing: no engine, no magnet add. Polling once a second or two is fine. The verdict changes when a window closes, or the moment playback is seen, whichever comes first; asking faster than the window is otherwise answered from the standing reading.
